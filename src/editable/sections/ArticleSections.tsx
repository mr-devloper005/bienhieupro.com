  ...
  return (
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-12 sm:pt-16 lg:pt-20`}>
        <div className={`rounded-2xl border ${pal.border} ${pal.darkBg} p-7 text-white shadow-[0_8px_32px_rgba(0,0,0,0.18)] sm:p-10 lg:p-14`}>
          ...
          <form action={basePath} className="mt-8 flex max-w-xl flex-col gap-3 sm:flex-row">
            ...
            <button className={`rounded-full ${pal.accentSoftBg} px-6 py-3 text-sm font-semibold ${pal.panelText}`}>Filter</button>
          </form>
        </div>
      </section>
      ...
      <section className={`${dc.shell.section} ${dc.shell.sectionY}`}>
        {posts.length ? (
          ...
          <div className={`${dc.surface.soft} p-8 text-center`}>
            <h2 className="text-2xl font-bold tracking-[-0.02em]">No articles found</h2>
            <p className={`mt-3 text-sm leading-7 ${pal.softMutedText}`}>Try another category or return to all articles.</p>
          </div>
        )}
        <div className="mt-10 flex flex-wrap items-center justify-center gap-3">
          {pagination.hasPrevPage ? <Link href={pageHref(page - 1)} className={`rounded-full border ${pal.border} bg-white px-5 py-3 text-sm font-semibold`}>Previous</Link> : null}
          <span className={`rounded-full ${pal.darkBg} px-5 py-3 text-sm font-semibold text-white`}>Page {page} of {pagination.totalPages || 1}</span>
          {pagination.hasNextPage ? <Link href={pageHref(page + 1)} className={`rounded-full border ${pal.border} bg-white px-5 py-3 text-sm font-semibold`}>Next</Link> : null}
        </div>
      </section>
    </main>
  )
  ...
  return (
    <main className={dc.shell.page}>
      <section className={`${dc.shell.section} pt-10 sm:pt-14 lg:pt-16`}>
        <div className={`grid gap-6 rounded-2xl border ${pal.border} bg-white p-6 shadow-[0_8px_32px_rgba(0,0,0,0.08)] lg:grid-cols-[minmax(0,1fr)_320px] lg:p-10`}>
          <div className="min-w-0">
            <Link href="/article" className={`inline-flex items-center gap-2 rounded-full border ${pal.border} px-4 py-2 text-sm font-semibold ${pal.panelText}`}><ChevronLeft className="h-4 w-4" /> Articles</Link>
            <p className={`${dc.type.eyebrow} mt-8 ${pal.accentText}`}>{voice.eyebrow}</p>
            <h1 className={`mt-4 max-w-4xl text-3xl font-bold leading-tight tracking-[-0.02em] ${pal.panelText} sm:text-4xl lg:text-5xl`}>{post?.title || pagesContent.detailPages.article.fallbackTitle}</h1>
          </div>
          <aside className={`min-w-0 rounded-xl ${pal.darkBg} p-6 text-white`}>
            ...
            <Link href="/contact" className={`mt-6 inline-flex items-center gap-2 rounded-full bg-white px-5 py-3 text-sm font-semibold ${pal.panelText}`}>Contact <ArrowRight className="h-4 w-4" /></Link>
          </aside>
        </div>
      </section>
      <section className="mx-auto w-full max-w-5xl px-4 pb-16 pt-6 sm:px-6 lg:px-8 lg:pb-24">
        <div className={`rounded-2xl border ${pal.border} bg-white p-6 shadow-[0_8px_32px_rgba(0,0,0,0.08)] sm:p-8 lg:p-10`}>
