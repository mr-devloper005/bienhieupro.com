    ...
    <Link href={href} className={`group block min-w-0 overflow-hidden ${dc.surface.dark} ${dc.motion.lift}`}>
      <div className="relative min-h-[520px] p-6 sm:p-8 lg:min-h-[620px]">
        ...
        <div className="relative z-10 flex h-full min-h-[460px] flex-col justify-end lg:min-h-[560px]">
          ...
          <h3 className="mt-5 max-w-3xl text-3xl font-bold leading-tight tracking-[-0.02em] sm:text-4xl lg:text-5xl">{post.title}</h3>
  ...
  return (
    <Link href={href} className={`group ${dc.layout.minRailCard} block overflow-hidden ${dc.surface.card} ${dc.motion.lift}`}>
      <div className={`${dc.media.frame} ${dc.media.ratio}`}>
        ...
        <span className={`absolute left-4 top-4 rounded-full ${pal.darkBg} px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-white`}>No. {String(index + 1).padStart(2, '0')}</span>
      </div>
      <div className="p-5">
        <p className={`${dc.type.eyebrow} ${pal.accentText}`}>{getEditableCategory(post)}</p>
        <h3 className={`mt-3 line-clamp-3 text-lg font-bold leading-tight tracking-[-0.02em] ${pal.panelText}`}>{post.title}</h3>
  ...
  return (
    <Link href={href} className={`group block min-w-0 ${dc.surface.soft} p-5 ${dc.motion.lift}`}>
      <div className="flex items-start gap-4">
        <span className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-full ${pal.darkBg} text-xs font-bold text-white`}>{index + 1}</span>
        <div className="min-w-0">
          ...
          <h3 className={`mt-2 line-clamp-2 text-lg font-bold leading-tight tracking-[-0.02em] ${pal.panelText}`}>{post.title}</h3>
  ...
  return (
    <Link href={href} className={`group grid min-w-0 gap-5 overflow-hidden ${dc.surface.card} p-4 ${dc.motion.lift} sm:grid-cols-[220px_minmax(0,1fr)]`}>
      ...
      <div className="min-w-0 p-2 sm:py-4 sm:pr-5">
        ...
        <h2 className={`mt-3 line-clamp-3 text-xl font-bold leading-snug tracking-[-0.02em] ${pal.panelText} sm:text-2xl`}>{post.title}</h2>
